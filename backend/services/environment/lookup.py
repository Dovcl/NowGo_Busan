...
from db.models import AirQualityCache, RipCurrentCache, UvIndexCache, WeatherCache, WeatherWarningCache
...
def _traffic_congestion_out(
    ...
    status:
      ...
      - 'normal': 도로 실측 (baseline 최소 관측 기준 통과)
    """
    ...
    # 도로 실측(source='road')은 계산 단계에서 이미 baseline 최소 관측 기준을 통과한 값이라 정식 표시
    status = "normal"
