...
# -------------------------------------------------------------------
# 2) 제목 기준 수동 override — 두 가지 서로 다른 이유로 cat3만으로는
#    env_type_code를 못 정하는 경우를 모아둔다. 여기서 나온 값도 반드시
#    위 5개 유형 중 하나다.
#
#    (a) 레포츠(contenttypeid=28): cat3의 기본 배정만으로는 "위치 특성"을
#        알 수 없는 경우(트래킹/캠핑 등은 해변·산·강 어디에도 있을 수 있음)
#        주소(addr1)를 보고 개별 확인한 결과. ※ 팀 내 최종 검토 필요(로컬
#        지리 확인 권장)
#    (b) TourAPI 원본 cat3 결측 랜드마크(해운대해수욕장 등): 실측 확인
#        (2026-09-12) 결과 이 항목들은 cat3 필드 자체가 TourAPI 원본에서
#        빈 문자열이라 cat3 매핑이 아예 안 먹힌다(URBAN으로 fallback됨).
#        `etl/fetch_missing_landmarks.py`가 별도 화이트리스트로 조회해오는
#        항목들이라 여기서 이름 기준으로 실제 유형을 보정한다
# -------------------------------------------------------------------
LOCATION_OVERRIDE_BY_TITLE = {
    # --- (b) cat3 결측 랜드마크, etl/fetch_missing_landmarks.py 참고 ---
    "해운대해수욕장": "BEACH",
    "광안리해수욕장": "BEACH",
    "송정해수욕장": "BEACH",
    "다대포해수욕장": "BEACH",
    "일광해수욕장": "BEACH",
    "임랑해수욕장": "BEACH",
    "태종대": "BEACH",        # 해안절경/전망(lclsSystm3=NA020800) — 기존 해안절경(A01011100) 선례와 일치
    "이기대": "BEACH",        # 해안 산책로/공원 — 기존 "해안누리길 몰운대길" 등 선례와 일치
    "부산 감천문화마을": "URBAN",  # 문화마을 — 기존 민속마을(A02010600) 선례와 일치
    "용두산공원": "URBAN",    # 도심 공원+전망탑(부산타워)

    # --- (a) 레포츠(contenttypeid=28) 위치 확정 ---
    "광안리 SUP Zone": "BEACH",
    "광안리 해양레포츠센터": "BEACH",
    "구덕야영장": "MOUNTAIN",
    "구포무장애숲길": "MOUNTAIN",
...
DIRECT_WATER_CONTACT_OVERRIDE_BY_TITLE = {
    # cat3 결측 랜드마크(위 LOCATION_OVERRIDE_BY_TITLE (b) 참고) 중 해수욕장 6곳 —
    # 수영 가능한 해변이라 직접 접촉. 태종대/이기대는 해안절벽·산책로라 접촉 없음(기본값 False 유지)
    "해운대해수욕장": True,
    "광안리해수욕장": True,
    "송정해수욕장": True,
    "다대포해수욕장": True,
